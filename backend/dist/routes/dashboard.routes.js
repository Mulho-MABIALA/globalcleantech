...
router.get('/rapport-mensuel', async (req, res) => {
    const moisParam = req.query.mois;
    let debut, fin;
    ...
    const [candidatures, demandesTotal, placements, messagesTotal, candidaturesParStatut, demandesParService, topPostes,] = await Promise.all([
...
router.get('/rapport-mensuel/csv', async (req, res) => {
    const moisParam = req.query.mois;
    let debut, fin;
    if (moisParam && /^\d{4}-\d{2}$/.test(moisParam)) {
        const [y, m] = moisParam.split('-').map(Number);
    ...
    const rows = placements.map(p => `${p.id};${p.candidature.nomComplet};${p.candidature.posteSouhaite};${p.candidature.telephone};${p.demande?.nomRaisonSociale ?? ''};${new Date(p.dateDebut).toLocaleDateString('fr-FR')};${p.dateFin ? new Date(p.dateFin).toLocaleDateString('fr-FR') : ''};${p.salaire ?? ''}`).join('\n');
...
router.get('/newsletter/export', async (_req, res) => {
    ...
    const rows = subs.map(s => `${s.email};${s.nom ?? ''};${new Date(s.createdAt).toLocaleDateString('fr-FR')}`).join('\n');
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
...
router.put('/content/:cle', async (req, res) => {
