...
async function getStats(_req, res) {
    ...
    sept_jours.setDate(now.getDate() - 7);
    const trente_jours = new Date();
    trente_jours.setDate(now.getDate() - 30);
    const soixante_jours = new Date();
    soixante_jours.setDate(now.getDate() - 60);
    const [totalCandidatures, nouvellesCandidatures7j, candidaturesATraiter, candidaturesParPoste, candidaturesParStatut, totalDemandes, nouvellesDemandes7j, demandesNouvelles, demandesParService, totalMessages, messagesNonLus, nouveauxMessages7j, dernieresCandidatures, dernieresDemandes, derniersMessages, 
    // Tendance 30j vs 30j précédents
    candidatures30j, candidaturesPrev30j, demandes30j, demandesPrev30j, messages30j, messagesPrev30j,] = await prisma.$transaction([
    ...
    const par_poste = {};
    candidaturesParPoste.forEach(g => { par_poste[g.posteSouhaite] = g._count._all; });
    const par_statut = {};
